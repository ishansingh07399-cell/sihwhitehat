...
def get_coordinates(location_name):
    """
    Dynamically geocode any detected location.
    ...
    No hardcoded city/location mapping.
    ...
    try:
        ...
        print(
            f"Geocoding failed for '{location_name}': {e}"
        )

    # IMPORTANT:
    # Do not guess a location.
    # Do not place it at India's center.
    return None
...
async def analyze_data(
    ...
    if raw_text.strip():
        evidence_hashes["Raw Intelligence Text"] = hashlib.sha256(
    ...
    if raw_text.strip():
        ...
        for ent in cleaned_entities:
            ...
            for unique_ent in resolved_entities:
                ...
                if (
                    group != unique_group
                    and group != 'MISC'
                    and unique_group != 'MISC'
                ):
                    continue

                w1 = word.lower()
                w2 = unique_word.lower()

                if (
                    w1 == w2
                    or (len(w1) > 3 and w1 in w2)
                    or (len(w2) > 3 and w2 in w1)
                ):
        ...
        # Linear for readability

        for i in range(len(resolved_entities) - 1):

            G.add_edge(
                resolved_entities[i]['word'],
                resolved_entities[i + 1]['word'],
                color='#475569',
                weight=1,
                label="Co-occurrence"
            )
    ...
    if cdr_file and cdr_file.filename:
        ...
        evidence_hashes[
            f"CDR Upload ({cdr_file.filename})"
        ...
        if (
            'Caller' in df_cdr.columns
            and
            'Receiver' in df_cdr.columns
        ):
    ...
    if bank_file and bank_file.filename:
        ...
        evidence_hashes[
            f"Bank Ledger Upload ({bank_file.filename})"
        ] = hashlib.sha256(content).hexdigest()
        ...
        if all(
            col in df_bank.columns
            for col in ['Source', 'Target', 'Amount']
        ):

            target_counts = (
                df_bank[
                    df_bank['Amount'] < 10000
                ]
                .groupby('Target')
                .size()
            )
    ...
    if raw_text:
        ...
        try:
            ...
            candidate_labels = [
                ...
                "criminal conspiracy or coordinated planning",
                "illegal weapons or firearms"
            ]

            zs_result = zero_shot_classifier(
                raw_text,
                candidate_labels,
                multi_label=True
            )
            ...
            for label, score in zip(
                ...
                if score > 0.4:
                    ...
                    if label == "murder or lethal violence":
                        ...
                        ipc_predictions.append({
                            "section": "Arms Act 25",
                            "description": "Offences relating to arms",
                            "reason":
                                f"Intent Classification "
                                f"(Confidence: {score*100:.1f}%) "
                                f"detected illegal arms."
                        })
            ...
            print(
                f"Zero-shot classification error: {e}"
            )
    ...
    for node in G.nodes:
        ...
        if node in betweenness_scores:
            G.nodes[node]['broker'] = (
                betweenness_scores[node]
            )
